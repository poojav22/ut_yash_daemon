...
//         if (rc > 0) {
//             buf[rc] = '\0';  // Null-terminate the received message

//             // Log exactly what the server receives
//             syslog(LOG_INFO, "Received message: '%s'", buf);
//             printf("Received message: '%s'\n", buf);

//             // Check for CMD and CTL prefixes
//             if (strncmp(buf, "CMD ", 4) == 0) {
//                 char *command = buf + 4;
//                 syslog(LOG_INFO, "Executing command: %s", command);
//                 run_yash(client.sock, command, &client);  // Execute the command

//             } else if (strncmp(buf, "CTL ", 4) == 0) {
//                 // Handle control signals
//                 char signal = buf[4];  // Get the control signal

//                 if (signal == 'c') {
//                     syslog(LOG_INFO, "Received SIGINT (Ctrl+C) from client");
//                     kill(-client.job_list[client.job_count].pgid, SIGINT);

//                 } else if (signal == 'z') {
//                     syslog(LOG_INFO, "Received SIGTSTP (Ctrl+Z) from client");
//                     kill(-client.job_list[client.job_count].pgid, SIGTSTP);

//                 } else if (signal == 'd') {
//                     syslog(LOG_INFO, "Received EOF (Ctrl+D) from client, closing connection");
//                     close(client.sock);  // Close connection on Ctrl+D
//                     pthread_exit(NULL);  // Exit the thread after client disconnects

//                 } else {
//                     syslog(LOG_ERR, "Unknown control signal: '%c'", signal);
//                 }
//             } else {
//                 syslog(LOG_ERR, "Unknown message format: %s", buf);
//             }
...
void *EchoServe(void *input) {
    ...
    for (;;) {
        ...
        if (rc > 0) {
            ...
            // Protocol handling
            if (strncmp(buf, "CMD ", 4) == 0) {
                // Strip "CMD " prefix and execute the command
                char *command = buf + 4;  // Skip the first 4 characters ("CMD ")
                syslog(LOG_INFO, "Executing command: %s", command);
                run_yash(client.sock, command, &client);  // Execute the command

            } else if (strncmp(buf, "CTL ", 4) == 0) {
                // Handle control signals
                char signal = buf[4];  // Get the control signal (e.g., 'C' for SIGINT, 'Z' for SIGTSTP)
                if (signal == 'C') {
                    syslog(LOG_INFO, "Received SIGINT (Ctrl+C) from client");
                    kill(-client.job_list[client.job_count].pgid, SIGINT);  // Send SIGINT to the process group
                } else if (signal == 'Z') {
                    syslog(LOG_INFO, "Received SIGTSTP (Ctrl+Z) from client");
                    kill(-client.job_list[client.job_count].pgid, SIGTSTP);  // Send SIGTSTP to the process group
                } else {
                    syslog(LOG_ERR, "Unknown control signal: %c", signal);
                }
            ...
            // Send prompt back to the client after handling
            send(client.sock, "\n# ", 3, 0);
        } else {
            syslog(LOG_INFO, "Client disconnected");
            close(client.sock);
